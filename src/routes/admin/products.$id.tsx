...
export const Route = createFileRoute('/admin/products/$id')({
  component: AdminProductEditForm,
})
...
function AdminProductEditForm() {
  ...
  const { data: productsData, isLoading: loadingProduct } = useQuery({
    queryKey: ['admin-products', { limit: 100 }],
  ...
  useEffect(() => {
    if (productsData?.data) {
      const prod = productsData.data.find((p) => p.id === id)
      if (prod) {
        const rawCat =
          prod.category_id ||
          (typeof prod.category === 'object'
            ? prod.category?.id || prod.category?.slug || prod.category?.name
            : prod.category)
        const staticCat = getStaticCategory(rawCat)
        const resolvedCategoryId = staticCat?.id || prod.category_id || ''
        ...
        setForm({
          ...
          images: prod.images || (prod.image_url ? [prod.image_url] : []),
          sku: prod.sku || '',
          wash_care: prod.wash_care || '',
          featured: !!prod.featured,
          best_seller: !!prod.best_seller,
          new_arrival: !!prod.new_arrival,
          stock: (prod.stock_quantity ?? prod.stock ?? 15).toString(),
          in_stock: prod.status !== 'out_of_stock' && (prod.stock_quantity ?? prod.stock ?? 0) > 0,
          tags: prod.tags || [],
        })

        const pPrice = parseFloat(prod.price?.toString() || '0')
        const pOfferPrice = prod.offer_price ? parseFloat(prod.offer_price.toString()) : null
        if (pPrice && pOfferPrice) {
          const calculatedDiscount = Math.round(((pPrice - pOfferPrice) / pPrice) * 100)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: ['product', form.slug] })
  ...
  if (loadingProduct) {
    return <div className="p-8 text-center text-gray-500 font-nav text-sm">Loading product details...</div>
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-heading text-2xl font-700 text-gray-800">Edit Product</h1>
            <p className="text-gray-400 text-sm">Update product fields and configuration</p>
          </div>
        </div>
        <button
          type="button"
